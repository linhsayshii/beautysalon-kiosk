        ...
        <div className="goods-dialog-body">
          ...
          {tab === 'information' ? <>
            ...
            <section className="goods-form-section"><div className="goods-section-heading"><span><strong>{type === 'account_card' ? 'Giá bán, mệnh giá' : 'Giá bán, giá vốn'}</strong><small>Giá được đồng bộ sang bảng giá chung khi lưu.</small></span><i className="ph ph-caret-up" /></div><div className="goods-form-grid">
              <div className="goods-field"><label htmlFor="goods-sale-price">Giá bán</label><div className="input-suffix"><input id="goods-sale-price" type="number" min="0" step="1000" value={form.salePrice} onChange={(event) => update('salePrice', event.target.value)} /><span>đ</span></div>{errors.salePrice && <small className="field-error">{errors.salePrice}</small>}</div>
              {type === 'account_card' ? <div className="goods-field"><label htmlFor="goods-face-value">Mệnh giá sử dụng</label><div className="input-suffix"><input id="goods-face-value" type="number" min="1" step="1000" value={form.faceValue} onChange={(event) => update('faceValue', event.target.value)} /><span>đ</span></div>{errors.faceValue && <small className="field-error">{errors.faceValue}</small>}</div> : <div className="goods-field"><label htmlFor="goods-cost-price">Giá vốn</label><div className="input-suffix"><input id="goods-cost-price" type="number" min="0" step="1000" value={form.costPrice} onChange={(event) => update('costPrice', event.target.value)} /><span>đ</span></div>{errors.costPrice && <small className="field-error">{errors.costPrice}</small>}</div>}
            </div></section>
